...
func init() {
	...
	testProfiles = []*profile.Custom{profile.Default(), profile.RFC4880(), profile.RFC9580(), profile.PQC(), profile.Symmetric()}
	testProfileNames = []string{"Default", "RFC4880", "RFC9580", "PQC", "Symmetric"}
	initEncDecTest()
	initGenerateKeys()
	initArmoredKeys()
	initKeyRings()
